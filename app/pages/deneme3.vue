<template>
  <div class="min-h-screen flex flex-col overflow-hidden bg-[#0d0f14] text-[#e8ecf5] font-['Syne',sans-serif] p-2 gap-2">
    <!-- Top Bar -->
    <header class="flex items-center justify-between px-6 h-[52px] border border-[#252a38] bg-[#12151d] rounded-xl shrink-0 z-10 w-full">
    ...
    <div class="flex flex-1 overflow-hidden gap-2 pb-2 h-[calc(100vh-68px)]">
      <!-- TOOLBOX (Left Panel) -->
      <div class="flex flex-col overflow-hidden w-[280px] border border-[#252a38] rounded-xl bg-[#12151d] shrink-0 relative">
        <div class="flex items-center justify-between p-3 px-5 border-b border-[#252a38] shrink-0 bg-[#12151d] z-20">
      ...
      <!-- CANVAS (Middle Panel) -->
      <div class="flex flex-col flex-1 overflow-hidden bg-[#0a0c10] border border-[#252a38] rounded-xl relative"
           @dragover.prevent="isDragOver = true"
           @dragleave="isDragOver = false"
           @drop.prevent="onCanvasDrop($event)">

        <div class="absolute inset-0 pointer-events-none transition-all z-0"
             :class="{ 'bg-[#00e5ff]/5 outline-dashed outline-2 outline-[#00e5ff]/25 -outline-offset-8': isDragOver }"></div>

        <div class="flex flex-col h-full relative z-10 w-full">
          <div class="flex items-center justify-between p-4 px-6 border-b border-[#252a38] shrink-0 bg-[#0a0c10] shadow-sm">
            ...
            >
                ...
                <div v-for="(node, nIndex) in cluster.nodes" :key="node.id" class="border border-[#79b8ff]/40 rounded-xl bg-[#181c27] p-4 flex flex-col">
                   <div class="flex items-center justify-between mb-3 border-b border-[#252a38]/50 pb-2">
                     ...
                     <button class="text-[#ff4545] text-[14px] hover:bg-[#ff4545]/10 px-2 py-0.5 rounded mr-1" @click="cluster.nodes.splice(nIndex, 1); generateK8sYaml()">✕</button>
                   </div>
                   ...
                   >
                      ...
                      <div v-for="(pod, pIndex) in node.pods" :key="pod.id" class="border border-[#f8c555]/30 bg-[#1e2333] rounded px-2.5 py-2 flex items-start gap-2">
                         <span class="text-[16px] mt-0.5">📦</span>
                         <div class="flex flex-col flex-1">
                           <input v-model="pod.name" class="bg-transparent border-none text-[#f8c555] font-bold text-[12px] font-['JetBrains_Mono',monospace] outline-none w-full" @input="generateK8sYaml"/>
                         ...
                         <button class="text-[#ff4545] text-[12px] hover:text-white mr-1" @click="node.pods.splice(pIndex, 1); generateK8sYaml()">✕</button>
                      </div>
                   </div>
                </div>
            </div>
          ...
          <div v-if="services.length > 0" class="flex flex-col gap-3">
             ...
             <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
                <div
                    ...
                    :key="service._uid"
                    class="border rounded-xl cursor-pointer transition-all duration-150 overflow-hidden bg-[#181c27] hover:border-[#2e3549] flex flex-col"
                ...
                >
                  <div class="flex items-center gap-2.5 py-3 px-4 bg-[#12151d] border-b border-[#252a38]">
                    <span class="text-[20px] shrink-0">{{ service.icon }}</span>
                    <input
                        v-model="service.name"
                        class="flex-1 bg-transparent border-none border-b border-transparent text-[#e8ecf5] font-['JetBrains_Mono',monospace] text-[14px] font-bold outline-none py-0.5 px-1 transition-colors duration-150 focus:border-[#00e5ff]"
                        placeholder="service-name"
                        @click.stop
                        @input="generateYaml"
                    />
                    <button class="bg-transparent border-none text-[#525c75] cursor-pointer text-[14px] p-1 mr-1 rounded transition-all duration-150 shrink-0 hover:text-[#ff4545] hover:bg-[#ff4545]/10" @click.stop="removeService(index)">✕</button>
                  </div>

                  <!-- Expanded Config -->
                  <div v-if="activeService === service._uid" class="px-4 pb-4 pt-3 flex flex-col gap-3">
          ...
          <div v-if="k8sClusters.length === 0 && services.length === 0" class="absolute inset-0 flex items-center justify-center pointer-events-none mt-10">
      ...
      <div class="flex flex-col overflow-hidden w-[450px] bg-[#0d0f14] border border-[#252a38] rounded-xl shrink-0">
...
function onNodeDrop(cluster, event) {
  if (dragItem) {
     if (dragItem.type === 'node') {
        cluster.nodeCounter++
        cluster.nodes.push({
          id: `${cluster.id}-n${cluster.nodeCounter}`,
          name: `node-${cluster.nodeCounter}`,
          pods: []
        })
     } else {
        // If an image app is dropped into Node, make it a pod!
        const lastNodeIdx = cluster.nodes.length - 1
        if(lastNodeIdx >= 0) {
          cluster.podCounter++
          cluster.nodes[lastNodeIdx].pods.push({
            id: `${cluster.id}-p${cluster.podCounter}`,
            name: `pod-${dragItem.id || 'custom'}-${cluster.podCounter}`,
...
function onPodDrop(node, event) {
  if (dragItem && (dragItem.image || !dragItem.type || dragItem.type === 'service')) {
    // Traverse up to find the parent cluster to increment podCounter correctly
    // or fallback to random/k8sUidCounter if cluster not available in context directly
    k8sUidCounter++
    node.pods.push({
      id: `pod-local-${k8sUidCounter}`,
